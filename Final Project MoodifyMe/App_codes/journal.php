...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // If there are no errors, save entry to database and analyze emotions
    if(empty($entry_err)) {
        ...
        if($stmt = mysqli_prepare($conn, $sql)) {
            ...
            if(mysqli_stmt_execute($stmt)) {
                ...
                $data = json_encode(array('text' => $entry_text, 'threshold' => 0.5));
                ...
                if ($result !== FALSE) {
                    // Process the predicted emotions from the API
                    $response = json_decode($result, true);
                    
                    // We still store emotions, but we don't show them in the journal page
                    // Store emotions in database - now only processing the top 2 emotions from API
                    foreach($response['emotions'] as $emotion_data) {
                            ...
                            if(mysqli_stmt_execute($emotion_stmt)) {
                                ...
                                if(mysqli_stmt_num_rows($emotion_stmt) > 0) {
                                    ...
                                    if($insert_emotion_stmt = mysqli_prepare($conn, $insert_emotion_sql)) {
                                        mysqli_stmt_bind_param($insert_emotion_stmt, "iid", $entry_id, $emotion_id, $confidence);
                    ...
                    // Generate recommendations based on the top emotion
                    if (!empty($response['emotions'])) {
                        $top_emotion = $response['emotions'][0]['emotion'];
                        
                        // Select a random song matching the top emotion
                        $song_sql = "SELECT song_id FROM songs WHERE mood = ? ORDER BY RAND() LIMIT 1";
                        $song_stmt = mysqli_prepare($conn, $song_sql);
                        mysqli_stmt_bind_param($song_stmt, "s", $top_emotion);
                        mysqli_stmt_execute($song_stmt);
                        mysqli_stmt_store_result($song_stmt);
                        
                        if(mysqli_stmt_num_rows($song_stmt) > 0) {
                            mysqli_stmt_bind_result($song_stmt, $song_id);
                            mysqli_stmt_fetch($song_stmt);
                            
                            // Insert song recommendation
                            $song_rec_sql = "INSERT INTO user_recommendations (user_id, content_type, content_id) VALUES (?, 'song', ?)";
                            
                            $song_rec_stmt = mysqli_prepare($conn, $song_rec_sql);
                        ...
                        mysqli_stmt_close($song_stmt);
                        
                        // Select a random movie matching the top emotion
                        $movie_sql = "SELECT movie_id FROM movies WHERE mood = ? ORDER BY RAND() LIMIT 1";
                        $movie_stmt = mysqli_prepare($conn, $movie_sql);
                        mysqli_stmt_bind_param($movie_stmt, "s", $top_emotion);
                        mysqli_stmt_execute($movie_stmt);
                        mysqli_stmt_store_result($movie_stmt);
                        
                        if(mysqli_stmt_num_rows($movie_stmt) > 0) {
                            mysqli_stmt_bind_result($movie_stmt, $movie_id);
                            mysqli_stmt_fetch($movie_stmt);
                            
                            // Insert movie recommendation
                            $movie_rec_sql = "INSERT INTO user_recommendations (user_id, content_type, content_id) VALUES (?, 'movie', ?)";
                            
                            $movie_rec_stmt = mysqli_prepare($conn, $movie_rec_sql);
                        ...
                        mysqli_stmt_close($movie_stmt);
                    }
                } else {
                    // API call failed, fall back to random emotion assignment
                    $emotions = array('joy', 'sadness', 'anger', 'fear', 'surprise', 'love', 'optimism');
                    // Only use 2 emotions as fallback since we only want 2 emotions now
                    $assigned_emotions = array_rand(array_flip($emotions), min(2, count($emotions)));
                ...
                // Set success message - removed mention of emotion analysis
                $success_msg = "Journal entry saved successfully! We've created some recommendations for you.";
            } else {
        ...
        <div class="container">
            <div class="journal-container">
                <div class="journal-header">
                    <h2 class="journal-title"><i class="fas fa-book-open"></i> My Journal</h2>
